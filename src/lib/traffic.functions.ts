...
export const getTrafficWorkspace = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const [nodes, roads, alerts, searches, saved, favorites, profile, roles] = await Promise.all([
    ...
    const error = [nodes, roads, alerts, searches, saved, favorites, profile, roles].find((result) => result.error)?.error;
    if (error) throw new Error("Unable to load the traffic workspace.");
    return { nodes: nodes.data ?? [], roads: roads.data ?? [], alerts: alerts.data ?? [], searches: searches.data ?? [], saved: saved.data ?? [], favorites: favorites.data ?? [], profile: profile.data, isAdmin: roles.data?.some((item) => item.role === "admin") ?? false };
  });
